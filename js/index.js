...
function addSite() {
  if (siteName.value != '' && inputValidation(siteUrl)) {
    var siteObject = {
      name: siteName.value,
      url: siteUrl.value,
    };
  ...
  for (var i = 0; i < arr.length; i++) {
    container += `
        <tr>
            <td scope="col">${i + 1}</td>
            <td>${arr[i].name}</td>
            <td><a href="${arr[i].url}" target="_blank" class="btn green-color-btn"><i class="fa-solid fa-eye me-2"></i>Visit</a></td>
...
function searchForBootmark(searchKey) {

  var result = []

  for (var i = 0; i < sitesList.length; i++) {
    if (sitesList[i].name.toLowerCase().includes(searchKey.trim().toLowerCase())) {
